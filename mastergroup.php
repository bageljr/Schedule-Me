...
<html>
	<head>
		...
		function pSchedule(){
			...
			for (i=0; i<(endCell-startCell); i++) {
				var cell = parseInt(startCell) + i;
				document.getElementById(cell).classList.add('selected10');
				if (eventArray[cell-1].numEvents == 0) {
					//document.getElementById(cell).innerHTML = eventName;
					document.getElementById(cell).classList.add('selected10');
					eventArray[cell-1].eventNames[0] = eventName;
			...
			//Can be used to post data to external database
			$.ajax({
				url:'http://localhost/~nolanjelinski/pscheduleevents.php',
				type:'post',
				data:data,
				success:function(){
					//alert("worked");
		...
		function pSubForm (){
			...
			//Can be used to post data to external database
			$.ajax({
				url:'http://localhost/~nolanjelinski/insertpData.php',
				type:'post',
				data:data,
				success:function(){
					//alert("worked");
		...
		function submitGroup() {
			...
			$.ajax({
				url:'http://localhost/~nolanjelinski/createGroup.php',
				type:'post',
				data: data,
				success:function(){
				}
	...
	<body>
		...
		<p1> 
			...
			<br>
			<button type="button" onclickk="showGroup()"> Make Group </button>
			<br>
			<div id="eventForm">
				<form id="Form">
					Start Time:<br>
			...
			<div id="popupForm">
				<form id="pForm">
					...
					<br>
					<input type="hidden" name="pgroupname" value="<?php echo $_GET['groupName'] ?>">
					<br><br>
					<input type="button" value="Submit" onclick="pSchedule()">
				</form>
